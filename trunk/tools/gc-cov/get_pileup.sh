...
bwa aln $prefix $reads 2> $out/aln.err | bwa samse $prefix - $reads 2> $out/samse.err  | samtools view -bhS - 2> $out/view.err | samtools sort -o - $out/$base.sort 2> $out/sort.err | samtools mpileup -f $ref -  
