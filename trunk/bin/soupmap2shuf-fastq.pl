...
if (scalar(@ARGV) != 4) {
	print STDOUT " Usage: ".basename($0)." <read 1 mapping> <read 2 mapping> <output filtered map> <output filtered read pairs FastQ>\n";
	exit 1;
...
open( FILTEREDREADS, ">".$ARGV[3] ) || die "Unable to open \"$ARGV[3]\" for writing\n";
...
while( my $line = <READMAP2> )
{
	...
	my @mapline1 = split( /\s+/, $keepreads{$mapline[0]} );
        print FILTEREDREADS "\@$mapline1[0]/1\n";
	my $seq = $mapline1[9];
	$seq = revcomp($seq) if($mapline1[1] == 16);
        print FILTEREDREADS "$seq\n";
        print FILTEREDREADS "+$mapline1[0]/1\n";
        print FILTEREDREADS "$mapline1[10]\n";
	$seq = $mapline[9];
	$seq = revcomp($seq) if($mapline[1] == 16);
        print FILTEREDREADS "\@$mapline[0]/2\n";
        print FILTEREDREADS "$seq\n";
        print FILTEREDREADS "+$mapline[0]/2\n";
        print FILTEREDREADS "$mapline[10]\n";
