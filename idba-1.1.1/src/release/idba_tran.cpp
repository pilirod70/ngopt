...
struct IDBAOption
{
    ...
    string connection_file(int kmer_size)
    { return directory + FormatString("/connection-%d", kmer_size); }

    string transcript_path_file(int kmer_size)
    { return directory + FormatString("/transcript-path-%d", kmer_size); }
...
void FindIsoforms(ContigGraph &contig_graph, deque<Sequence> &transcripts, deque<ContigGraphPath> &transcript_paths);
...
void Assemble(HashGraph &hash_graph)
{
    ...
    if (!option.is_no_coverage)
    {
        ...
        contig_graph.IterateComponentCoverage(option.min_contig, ratio, min_cover, 1e100, 1.1, max_component_size);
        contig_graph.GetContigs(multi_contigs, multi_contig_infos);
    ...
    deque<ContigGraphPath> transcript_paths;

    FindIsoforms(contig_graph, transcripts, transcript_paths);
    ...
    transcript_paths.resize(index);

    PrintN50(transcripts);
    WriteSequence(option.transcript_file(kmer_size), transcripts, FormatString("transcript-%d", kmer_size));
    ofstream fout(option.transcript_path_file(kmer_size).c_str(), ios_base::binary | ios_base::out);
    for (unsigned i = 0; i < transcript_paths.size(); ++i)
    {
        ContigGraphPath &path = transcript_paths[i];
        for (unsigned j = 0; j < path.num_nodes(); ++j)
            fout << path[j].id() << "_" << path[j].is_reverse() << "_" << path[j].contig_size() << "_" << path[j].kmer_count() << " ";
        fout << endl;
    }
}
...
void FindIsoforms(ContigGraph &contig_graph, deque<Sequence> &transcripts, deque<ContigGraphPath> &transcript_paths)
{
    ...
    FILE *fconn = OpenFile(option.connection_file(kmer_size), "wb");
    for (unsigned i = 0; i < component_strings.size(); ++i)
        fprintf(fconn, "%s\n", component_strings[i].c_str());
    fclose(fconn);
              ...
              if (components[i][j].in_edges().size() == 0)
                {
                    ...
                    for (unsigned k = 0; k < isoforms.size(); ++k)
                    {
                        ...
                        transcript_paths.push_back(isoforms[k]);
