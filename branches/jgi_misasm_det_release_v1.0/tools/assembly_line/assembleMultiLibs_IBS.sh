...
if [ $# -lt 5 ] || [ $mod2 -eq 0 ] ; then
	echo "Usage: `basename $0` <output_base> <kcut> <mnpair> <lib1> <ins1> <lib2> <ins2> . . . <libN> <insN>"
...
#	echo "Final assembly under <output_base>_2stg/scaf_lg/scaffolds_h${h}_L${L}_w<mnpair>.fasta"
	...
	while [ $x -le $(($# - 1)) ] ; do 
		...
		echo "Executing the following command: scaf_sopra_bwa.sh $dir $contigs ${!lib} $w ${!ins} >> $stdout 2>> $stderr" 
		scaf_sopra_bwa.sh $dir $contigs ${!lib} $w ${!ins} >> $stdout 2>> $stderr 
		x=$(( $x + 2 ))
		stage=$(( $stage + 1 ))
		contigs=$dir/scaffolds_h${h}_L${L}_w$w.fasta
		dir=$a/stage$stage
	done
